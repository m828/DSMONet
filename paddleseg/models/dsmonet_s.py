...
class DSMONet_S(nn.Layer):
    ...
    def __init__(self,
        ...
        self.cm = DAPPM(backbone_out_chs[-2], out_ch, out_ch)
        self.cms = DAPPM(backbone_out_chs[-1], out_ch, out_ch)
    ...
    def forward(self, x):
        ...
        high_feats = self.cm(feats_selected[-2])
        high_feat_1024 = self.cms(feats_selected[-1])
        high_feat = self.arm1(feats_selected[-2], high_feats)
        head_seg3 = high_feat
        ...
        # 细节指导语义
        high_feat_1024 = F.upsample(high_feat_1024, paddle.shape(feats_selected[-2])[2:])
        high_feats = high_feat + seg_body + high_feat_se + high_feat_1024
